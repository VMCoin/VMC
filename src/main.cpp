...
uint256 hashGenesisBlock("0xa5ec3d6b618a8998a97edc69e94fae8dba19de68d2883de40cb5449361327516");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // VirtualminingCoin: starting difficulty is 1 / 2^12
...
int64 nChainStartTime = 1397523223; // Line: 2815
...
const string strMessageMagic = "VirtualminingCoin Signed Message:\n";
...
{
    // VirtualminingCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 171.2 * COIN;
    if (nHeight == 1)
    {
        nSubsidy = 10000000 * COIN; //Premine for the people of catalonia
    }
    else if (nHeight > 1 )
    {
        nSubsidy = 171.2 * COIN; //No insta Mining
...
static const int64 nTargetTimespan = 5 * 60; // VirtualminingCoin: 3.5 days
static const int64 nTargetSpacing = 60; // VirtualminingCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // VirtualminingCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
                if (pindexLast->nHeight+1 >= 2) { DiffMode = 2; } // VirtualminingCoin, 100 blocks after first legacy diff adjustment
        }
        else {         
        	if (pindexLast->nHeight+1 >= 2) { DiffMode = 2; }  //VirtualminingCoin, 5 days after 27/01/2014 12:00 UTC
        }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // VirtualminingCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // VirtualminingCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "booobooob dasdasdasjhhfjdhOMGRkkkOOKERCRYPTOFACTORY";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        txNew.vout[0].nValue = 171.2 * COIN;
        ...
        block.nNonce = 2153241;
        block.nTime = 1396356073;
        ...
        assert(block.hashMerkleRoot == uint256("0x6b1f01f8c08337d987778fb0a0e8827d287dc57c5fca8a9c6c3d46b7c29fe97c"));
...
unsigned char pchMessageStart[4] = { 0xfa, 0xbf, 0xb5, 0xda }; // VirtualminingCoin: increase each by adding 1 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VirtualminingCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VirtualminingCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VirtualminingCoinMiner(CWallet *pwallet)
{
    printf("VirtualminingCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("VirtualminingCoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running VirtualminingCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("VirtualminingCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VirtualminingCoinMiner, pwallet));
}
